...
<?php 
	...
	if( $xType == 'products' || $xType != 'realestates' || $xType != 'cars' || $xType != 'rental') {  
	
		$salesMan = Doctrine::getTable('User')->find($product->getUserId()); 
	
		$categoryId = myConstants::getCategoryId($product->getCategoryType());
		
		$products = Doctrine::getTable('Product')->getUserOtherProducts($product->getUserId(), $product->getId(), $categoryId,6); ?>
